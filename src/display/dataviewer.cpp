                                          ...
                                          m_filterColumnNames{}, QWidget(parent)
{
  ...
  auto *refreshShortcut = new QShortcut(QKeySequence::Refresh, this, nullptr, nullptr, Qt::WidgetWithChildrenShortcut);
  connect(refreshShortcut, &QShortcut::activated, this, &DataViewer::refresh);
}
...
void DataViewer::refresh()
{
  ...
  updateNavigation();
  emit refreshed();
  qDebug() << objectName() << "refreshed";
}

void DataViewer::setFilter(const QString &filter)
{
  if (m_filterColumnNames.isEmpty())
    return;
  QStringList filters;
