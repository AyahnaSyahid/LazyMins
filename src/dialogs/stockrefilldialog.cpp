...
void StockRefillDialog::on_simpanButton_clicked() {
  ...
  if (notes.isEmpty())
    err << "- Berikan keterangan spesifik yang bersangkutan dengan masuknya "
           "barang";
  ...
  if (err.size() > 0) {
    QMessageBox::warning(
        this, "Input bermasalah",
        QString("Mohon untuk mengisi data sesuai dengan kriteria :\n%1")
            .arg(err.join("\n")));
    return;
  }
