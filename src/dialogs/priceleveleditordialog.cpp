...
namespace {
  class PriceLevelDelegate : public QStyledItemDelegate 
  {
    public:
      ...
      void setModelData(QWidget *editor, QAbstractItemModel *md, const QModelIndex& ix) const override {
        if (ix.column() == 1) {
            auto sp = qobject_cast<QSpinBox*>(editor);
            if (sp) {
              QVariant minData = ix.data(PriceLevelEditorModel::DefaultPriceRole);
              // if(minData.toInt() == sp->value()) return; // just dont set data if value same as minData
...
PriceLevelEditorDialog::PriceLevelEditorDialog(QWidget *p):
  ui(new Ui::PriceLevelEditorDialog), m_model(new PriceLevelEditorModel(this)), QDialog(p)
{
  ui->setupUi(this);
...
bool PriceLevelEditorDialog::saveAll() {
  ...
  if (ui->costSpinBox->value() != m_defaultCost) {
    ProductManager pmgr;
    bool updateOk = pmgr.update(m_productId, {{"cost_price", ui->costSpinBox->value()}});
    if (!updateOk) {
      QMessageBox::information(this, "Kesalahan", 
        QString("Update harga default produk gagal:\n%1").arg(pmgr.errorString()));
      return false;
    }
...
void PriceLevelEditorDialog::on_tableView_customContextMenuRequested(const QPoint& p)
{
  QMenu ctx;
  ctx.setToolTipsVisible(true);
  auto newLevel = ctx.addAction("Level baru");
  newLevel->setToolTip("Buat level baru");
  connect(newLevel, &QAction::triggered, this, &PriceLevelEditorDialog::onCreateNewLevel);
...
void PriceLevelEditorDialog::onCreateNewLevel() {
  ...
  connect(&pld, &QDialog::accepted, [this]() {
    m_model->setProductId(m_productId);
  } );
  pld.exec();
}