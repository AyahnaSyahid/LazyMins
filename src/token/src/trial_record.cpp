...
static constexpr const unsigned char TRIAL_HMAC_SECRET[] = {
    0x5A, 0x7B, 0x3C, 0x9D, 0xE1, 0xF2, 0x4A, 0x6B, 0x8C, 0x0D, 0xE3,
    0xF5, 0x7A, 0x9B, 0x1C, 0x3E, 0x4F, 0x6D, 0x8A, 0x0B, 0x2C, 0x4E,
    0x6F, 0x8D, 0x0A, 0x2B, 0x4C, 0x6E, 0x8F, 0x01, 0x23, 0x45};
...
static fs::path getTrialRecordPath() {
  // Coba XDG_DATA_HOME (Linux), APPDATA (Windows), atau ~/Library/Application
  // Support (macOS) Fallback ke ~/.local/share atau ~/AppData/Roaming
  std::string baseDir;
...
static std::string hmacSha256Hex(const std::string& data) {
  ...
  if (!HMAC(EVP_sha256(), TRIAL_HMAC_SECRET, TRIAL_HMAC_SECRET_LEN,
...
std::string writeTrialRecord(std::string_view installDate) {
  ...
  if (!out) {
    ...
    throw std::runtime_error("Gagal menulis file trial record: " +
                             filePath.string());
  }
