...
class DailySalesRenderer : public ReportRendererBase {
 public:
  ...
  qreal render(qreal startY = 0.0) override {
    ...
    y = renderDocHeader(m_data.company, m_data.meta, "Laporan Penjualan Harian",
                        period, y);
    y += 8;
 ...
 private:
  ...
  qreal renderOrderTable(qreal y) {
    ...
    addTableRow(cols, {"Total", "", "", "", formatRp(total), ""}, y, false,
                true);
  ...
  qreal renderPaymentTable(qreal y) {
    ...
    addTableRow(
        cols,
        {"Total diterima", QString::number(grandCount), formatRp(grandTotal)},
        y, false, true);
    return y + m_theme.rowHeight + 4;
  }
  ...
  qreal renderTopProductsTable(qreal y) {
    ...
    for (const auto& row : m_data.topProducts) {
      y = addTableRow(cols,
                      {row.productName, row.categoryName, row.qtySold,
