...
BaseManager::BaseManager(const QString& tableName, bool useSoftDelete)
    : m_tableName(tableName), m_useSoftDelete(useSoftDelete)
{
}
...
QSqlRecord BaseManager::create(const QVariantMap& params)
{
    ...
    if (query.exec()) {
        ...
        QSqlRecord record = getById(lastId);
...
QSqlRecord BaseManager::getById(int id)
{
    QSqlRecord record;
    QSqlQuery query(BaseManager::connection);
