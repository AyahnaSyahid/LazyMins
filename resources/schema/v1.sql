...
CREATE TABLE
    orders (
        ...
        staging_status TEXT DEFAULT 'pending' COLLATE NOCASE CHECK (
            staging_status IN (
                'pending',
                'processing',
                'ready',
...
CREATE TABLE
    order_items (
        ...
        subtotal INTEGER GENERATED ALWAYS AS ( -- Subtotal = (quantity * sale_price * size_width * size_height)
            CAST(
                (
                    (quantity * sale_price * size_width * size_height) + 99.99999
                ) / 100 AS INT
            ) * 100
        ) VIRTUAL,
        ...
        total INTEGER GENERATED ALWAYS AS (
            ...
            ) + COALESCE(finishing_total, 0) - COALESCE(discount_amount, 0)
        ) VIRTUAL, -- Total =  Subtotal - ( discount + finishing )
...
CREATE TABLE
    payments (
        ...
        cash_change INTEGER,
        verification_status TEXT DEFAULT 'pending' COLLATE NOCASE CHECK (
            verification_status IN ('pending', 'verified', 'cancelled')
        ),
        notes TEXT,
        admin_id INTEGER NOT NULL,
...
CREATE TABLE
    akun_transaksi (
        ...
        tipe TEXT NOT NULL COLLATE NOCASE CHECK (
            tipe IN (
                'cash', -- Uang tunai
                'bank', -- Rekening bank
...
CREATE TABLE
    invoices (
        ...
        staging_status TEXT DEFAULT 'draft' COLLATE NOCASE CHECK (
            staging_status IN ('draft', 'issued', 'sent', 'cancelled')
        ),
        settlement_status TEXT DEFAULT 'unpaid' COLLATE NOCASE CHECK (
            settlement_status IN ('unpaid', 'partial', 'paid', 'refunded')
        ),
...
SELECT
    ...
    CASE
        WHEN p.use_area = 0 THEN SUM(oi.quantity)
        ELSE SUM(oi.quantity * oi.size_width * oi.size_height)
    END AS total_sold,
...
FROM
    konsumen k
    LEFT JOIN orders o ON o.customer_id = k.id
    AND o.staging_status != 'cancelled' -- jangan hitung order batal
WHERE
    k.is_active = 1
GROUP BY
    k.id,
