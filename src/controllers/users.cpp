...
QSqlRecord UserController::getUserRecord(const QString &username) const
{
  QSqlRecord rec;
  QSqlQuery q(BaseManager::connection);
...
int UserController::getRoleId(int id) const
{
    return getUserRecord(id).value("role_id").toInt();
}
