...
DBOperationHelper::OperationResult DBOperationHelper::createInstantOrder(
  ...
  // create order first
  auto opt_ord =
      om.create({{"admin_id", _admin_id},
                 {"order_number", header.order_number},
                 {"customer_id", header.customer_id < 1
  ...
  for (auto const& order_item : items) {
    ...
    if (order_item.use_area) {
      c_qty =
          roundUpValue(c_qty * order_item.size_width * order_item.size_height);
    }

    auto opt_sm = smm.recordMovement(order_item.product_id, "out", -c_qty,
                                     pr_stock, getAdminId(), "order_items",
...
DBOperationHelper::OperationResult DBOperationHelper::refillProductStock(
  ...
  auto opt_sm = smm.recordMovement(productId, "in", stockIn, current_stock,
                                   currentAdminId(), "", 0,
                                   "Supplier : " + supplier + "\n" + notes);
...
DBOperationHelper::OperationResult DBOperationHelper::loadInvoiceData(
  ...
  rec->date =
      opt_inv->value("created_at").toDateTime().date().toString("dd/MM/yyyy");
  rec->time =
      opt_inv->value("created_at").toDateTime().time().toString("HH:mm");
  ...
  qDebug() << "[DB] Invoice Header loaded:" << rec->invoiceNo
  ...
  for (auto const& order : order_list) {
    ...
    for (auto const& item : items) {
      ...
      qDebug() << "[DB] Added Item:" << rItem.description
               << "Qty:" << rItem.quantity;
...
DBOperationHelper::OperationResult DBOperationHelper::loadInvoiceDataFast(
  ...
  rec->companyName =
      aps.getSettings("company_name")["setting_value"].toString();
  rec->companyAddress =
      aps.getSettings("company_address")["setting_value"].toString();
  rec->companyPhone =
      aps.getSettings("company_phone")["setting_value"].toString();
  rec->companyEmail =
      aps.getSettings("company_email")["setting_value"].toString();
  ...
  rec->date = rec_invoice.value("created_at")
                  .toDateTime()
                  .date()
                  .toString("dd/MM/yyyy");
  rec->time =
      rec_invoice.value("created_at").toDateTime().time().toString("HH:mm");
  rec->cashierName = adminName;
  ...
  for (auto const& order : rec_orderList) {
    ...
    for (auto const& item : items) {
      ...
      rItem.description = QString("[%1] %2").arg(
          ...
          item.value("product_name").toString().mid(0, 23));
      rItem.quantity = item.value("quantity").toDouble();
