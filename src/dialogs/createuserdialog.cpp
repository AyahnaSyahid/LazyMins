...
bool CreateUserDialog::checkInputs() {
  QList<QLineEdit*> lineEdits{ui->fullnameEdit,  ui->phoneEdit,
                              ui->emailEdit,     ui->usernameEdit,
                              ui->passwordEdit1, ui->passwordEdit2};

  for (auto* edit : lineEdits) {
    if (edit->text().isEmpty()) {
      edit->setStyleSheet("background-color: rgb(255, 240, 181);");
      QMessageBox::information(this, "Periksa Input",
  ...
  if (ui->passwordEdit1->text() != ui->passwordEdit2->text()) {
    ...
    QMessageBox::information(this, "Periksa Password",
                             "Verifikasi password yang anda masukkan tidak "
                             "sesuai dengan field password diatasnya");
  ...
  if (ui->passwordEdit1->text().size() < 6) {
    QMessageBox::information(
        this, "Periksa Password",
        "Password terlalu pendek, minimal jumlah karakter adalah 6");
...
void CreateUserDialog::on_simpanButton_clicked() {
  ...
  if (am.exists(ui->usernameEdit->text())) {
    QMessageBox::information(this, "Maaf",
                             "Username ini telah digunakan admin lain\nCoba "
                             "gunakan Username yang lain");
    return;
  }

  QVariantMap param{{"username", QVariant(ui->usernameEdit->text())},
  ...
  QMessageBox::information(this, "Kesalahan",
                           "Gagal menambahkan user kedalam database");
};