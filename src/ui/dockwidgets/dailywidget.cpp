...
ui(new Ui::DailyWidget), db(_d), QWidget(parent) {
    ...
    connect(model, &QSqlTableModel::modelReset, this, &DailyWidget::reloadData);
    connect(model, &QSqlTableModel::dataChanged, this, &DailyWidget::reloadData);
...
void DailyWidget::reloadData() {
    ...
    // hitung jumlah order yang dibuat hari ini oleh user
    q.prepare("SELECT COUNT(*) FROM orders WHERE user_id = ? AND date(created_utc) = date('now')");
    q.addBindValue(user.value("user_id"));
    q.exec();
    if(q.next()) {
      ui->label_ordersSavedCount->setText(locale().toString(q.value(0).toInt()));      
    } else {
      ui->label_ordersSavedCount->setText("Gagal Menghitung");      
    }
    // hitung nilai order yang dibuat hari ini oleh user
    q.prepare(R"--( SELECT SUM(oc.price - oc.discount) AS total_order
                      FROM orders_calc oc
                      JOIN orders od USING (order_id)
                     WHERE od.user_id = ? AND date(od.created_utc) = date('now') )--");
    q.addBindValue(user.value("user_id"));
    q.exec();
    if(q.next()) {
      ui->label_ordersSavedValueSum->setText(locale().toString(q.value(0).toInt()));
    } else {
      ui->label_ordersSavedValueSum->setText("Gagal Menghitung");      
    }
