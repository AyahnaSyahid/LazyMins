...
PriceLevelDialog::~PriceLevelDialog() { delete ui; }
...
bool PriceLevelDialog::setPriceLevelId(int pid) {
  ...
  if (!opt_level.has_value()) {
    QMessageBox::critical(
        this, "Kesalahan",
        QString(
            "Data tidak ditemukan,\nLevel Harga dengan ID %1 tidak tersedia.")
            .arg(pid));
    return false;
  }
...
void PriceLevelDialog::on_simpanButton_clicked() {
  ...
  if (existing.has_value()) {
    ...
    if (existingId != m_priceLevelId) {
      QMessageBox::warning(
          this, "Nama Sudah Ada",
          QString("Nama level '%1' sudah digunakan. Silakan gunakan nama lain.")
              .arg(name));
      return;
    }
  }
...
bool PriceLevelDialog::isDirty() const {
  // Jika ID -1 (Data Baru), otomatis dianggap kotor/dirty jika input tidak
  // kosong
  if (m_priceLevelId < 1) {
